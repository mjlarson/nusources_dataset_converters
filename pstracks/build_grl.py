...
def main(args):
    ...
    def blank_grl(filename):
        ...
        for line in allruns[1:]:
            splits = line.split(",")
            if ("GOOD,GOOD" not in line):
                continue
            if ("partial" in line.lower()) and (not args.allow_partial):
                continue
            run = int(splits[0])
    ...
    # ----------------------------------------------
    data = {}
    files = sorted(glob(join(args.indir, "*data*.npy")))
    files += sorted(glob(join(args.indir, "*exp*.npy")))
    for f in files:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--allow_partial", default=False, action="store_true", help="Allow good partial detector runs?")
